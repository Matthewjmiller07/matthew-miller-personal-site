/* Zmanim Board — WordPress plugin build
   theothermatthewmiller.com/zmanim-widget
   Renders into every element matching .zmanim-board.
   Options via data attributes: data-city="Jerusalem" data-max-width="420" */
(function () {
  'use strict';

  var BASE = 'https://theothermatthewmiller.com';
  var CFG  = window.ZMANIM_BOARD_CONFIG || {};
  var BG   = CFG.bgUrl || (BASE + '/images/zmanim-board-bg.png');
  ...
  function cityIdxFromLabel(label){
    if(!label)return 5; // New York
    var needle=String(label).toLowerCase();
    for(var i=0;i<CITIES.length;i++){
      if(CITIES[i].label.toLowerCase().indexOf(needle)!==-1)return i;
    }
    return 5;
  }
  ...
  function Widget(host){
    var S = {
      cityIdx: cityIdxFromLabel(host.getAttribute('data-city')),
      weekData:[], parasha:'', hebrewDate:'',
      loading:true, error:null, open:false,
      baseFont:10,   // px — set by ResizeObserver
    };
    var maxWidth = parseInt(host.getAttribute('data-max-width'),10) || 420;
    ...
    function boot(){
      ...
      wrap=el('div',{style:'position:relative;width:100%;max-width:'+maxWidth+'px;aspect-ratio:1086/1448;margin:0 auto;'});
    ...
    function renderPaper() {
      ...
      if(S.loading){
        ...
        var orbs=div('display:flex;gap:4px;');
        var msg=div('font-size:'+b+'px;color:#8b1a2b;direction:rtl;');
        msg.textContent='טוען זמנים…';
        ldg.appendChild(orbs); ldg.appendChild(msg);
  ...
  function injectStyles(){
    if(document.getElementById('zmanim-board-styles'))return;
    var s=document.createElement('style');
    s.id='zmanim-board-styles';
    s.textContent=[
      '.zmanim-board *{box-sizing:border-box;}',
      '.zmanim-board-city-btn:hover{opacity:.85;}',
      '.zmanim-board-city-btn:focus{outline:2px solid rgba(212,165,32,.6);outline-offset:2px;}',
    ].join('');
  ...
  // ── Boot all instances ────────────────────────────────────────────────────────
  function initAll(){
    var hosts=Array.prototype.slice.call(document.querySelectorAll('.zmanim-board'));
    if(!hosts.length)return;
    injectStyles();
    hosts.forEach(function(host){
      if(host.getAttribute('data-zmanim-board-init'))return;
      host.setAttribute('data-zmanim-board-init','1');
      new Widget(host);
    });
  }
